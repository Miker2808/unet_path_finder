...
from base_models import DoubleConv, AttentionBlock

class RESNET_UNET_ATTEN(nn.Module):
    def __init__(
            self, in_channels=3, out_channels=1, pretrained=True
    ):
        super(RESNET_UNET_ATTEN, self).__init__()
        self.name = "resnet_unet_atten"
...
if __name__ == "__main__":
    ...
    model = RESNET_UNET_ATTEN(in_channels=3, out_channels=1)
    preds = model(x)
